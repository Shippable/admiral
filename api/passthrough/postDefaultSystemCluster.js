...
function postDefaultSystemCluster(req, res) {
  ...
  async.series([
      ...
      _getWorkflowControllers.bind(null, bag),
...
function _getWorkflowControllers(bag, next) {
  var who = bag.who + '|' + _getWorkflowControllers.name;
  logger.verbose(who, 'Inside');

  var query = 'object=subscriptions';
  bag.shippableAPIAdapter.getWorkflowControllers(query,
    function (err, workflowControllers) {
      if (err)
        return next(who, ActErr.OperationFailed,
          'Failed to get workflowControllers:' + util.inspect(err));

      bag.grishamSubscriptionIds = _.pluck(workflowControllers, 'objectId');

      bag.subscriptionIds =
        _.difference(bag.subscriptionIds, bag.grishamSubscriptionIds);
